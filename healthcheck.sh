...
check_ntp()
{
    ...
    #NTP logs check
    test_output_error=0
    printf "|\t\t\t| NTP Logs\t\t\t|" | tee -a $output_log
    if [[ $(grep ntpdate /var/log/messages | wc -l) -ge 1 ]]; then
        if [[ $(echo $ntp_date | grep "step time server") ]]; then
            check_passed
            printf "NTP,NTP Logs,OK,\n" >> $csv_log
        elif [[ $(echo $ntp_date | grep "no server suitable") ]]; then
            check_failed
            printf "NTP,NTP Logs,WARNING,sk92602 and sk83820\n" >> $csv_log
            ntp_error=1
            printf "NTP Logs: NTP cannot find server for synchronization of the system clock.\n" >> $logfile
        fi
    elif [[ $(grep ntpd /var/log/messages | grep "synchronized to") ]]; then
        check_passed
        printf "NTP,NTP Logs,OK,\n" >> $csv_log
    else
        check_failed
        printf "NTP,NTP Logs,WARNING,sk92602 and sk83820\n" >> $csv_log
        ntp_error=1
        printf "NTP Logs: NTP entries not found in \"/var/log/messages\".\n" >> $logfile
    fi
...
check_memory()
{
    ...
    printf "\n\nMemory Info:\n" >> $full_output_log
    free -k  >> $full_output_log

    #Physical memory check
    total_mem=$(free -k | grep Mem | awk '{print $2}')
    free_mem=$(free -k | grep buffers | grep -v shared | awk '{print $4}')
    ...
    total_swap=$(free -k | grep Swap | awk '{print $2}')
    used_swap=$(free -k | grep Swap | awk '{print $3}')
    used_swap_percent=$(( 100 * used_swap / total_swap ))
    ...
    if [[ $sys_type == "GATEWAY" || $sys_type == "STANDALONE" ]]; then
        ...
        kernel_memory_failed=$(echo $fwctlpstat | grep -A5 "kmem" | grep Allocations | awk '{print $4}')
...
check_cpu()
{
    ...
    #Log usage
    printf "\n\nCPU information:\n" >> $full_output_log
    mpstat -P ALL | grep -v $(hostname) | grep -v all | sed '/^$/d' >> $full_output_log
    
    #idle check
    printf "| CPU\t\t\t| CPU idle%%\t\t\t|" | tee -a $output_log
    all_cpu_idle=$(mpstat -P ALL | grep -v $(hostname) | grep -v all | sed '/^$/d' | grep -v CPU | awk '{print $10}')
    current_cpu=0
    ...
    all_cpu_user=$(mpstat -P ALL | grep -v $(hostname) | grep -v all | sed '/^$/d' | grep -v CPU | awk '{print $3}')
    current_cpu=0
    test_output_error=0
    ...
    all_cpu_system=$(mpstat -P ALL | grep -v $(hostname) | grep -v all | sed '/^$/d' | grep -v CPU | awk '{print $5}')
    current_cpu=0
    test_output_error=0
    ...
    all_cpu_wait=$(mpstat -P ALL | grep -v $(hostname) | grep -v all | sed '/^$/d' | grep -v CPU | awk '{print $6}')
    current_cpu=0
    test_output_error=0
    ...
    all_cpu_interrupt=$(mpstat -P ALL | grep -v $(hostname) | grep -v all | sed '/^$/d' | grep -v CPU | awk '{print $8}')
    current_cpu=0
    test_output_error=0
...
check_connections_stats()
{
    ...
    connections_limit=$(fw tab -t connections | grep limit | grep -v Kernel | grep -v connections | awk '{print $35}')
...
check_cluster_statistics()
{
    ...
    if [[ "$cphaprob_stat" == *"High Availability"* ]]; then
        active_active_check=$(echo "$cphaprob_stat" | grep -v Mode | grep Active | wc -l)
    else
        active_active_check=0
    fi
    
    #Find the overall number of cluster members
    single_member_check=$(echo "$cphaprob_stat" | grep ^[1-8] | wc -l)
    ...
    #Display Warning messages if issue with cluster#
    printf "| Cluster\t\t| Cluster Status\t\t|" | tee -a $output_log
    if [[ $cluster_status == "Active" || $cluster_status == "Standby" ]]; then
        check_passed
        ...
        if [[ $single_member_check -eq 1 ]]; then
...
check_cp_software()
{
	...
	if [[ $cpinfo_build_version -ge 914000182 ]]; then
        ...
        printf "The cpinfo utility is up to date as of 11-08-2017.\n" >> $full_output_log
	else
        check_failed
    ...
    if [[ $cpuse_build_version -ge 1298 ]]; then
        check_passed
        printf "Check Point,CPUSE Build Number,OK,\n" >> $csv_log
        printf "CPUSE is up to date as of 11-08-2017.\n" >> $full_output_log
	else
        check_failed
